...
class XuxemonsUserController extends Controller
{
    ...
    public function debug(Request $request)
    {
        try {
            ...
            // Intentar obtener un Xuxemon aleatorio que el usuario no tenga
            $xuxemonAleatorio = null;
            do {
                $xuxemonAleatorio = self::obtenerXuxemonAleatorio();
                $xuxemonExistente = XuxemonsUser::where('xuxemon_id', $xuxemonAleatorio->id)
                    ->where('user_id', $user->id)
                    ->first();
            } while ($xuxemonExistente);

            // Si no se encuentra un Xuxemon aleatorio que el usuario no tenga, retornar un mensaje de error
            if (!$xuxemonAleatorio) {
                return response()->json(['message' => 'No se pudo encontrar un Xuxemon disponible'], 404);
            }
            ...
            return response()->json([$nuevoXuxemonUsuario], 200);
        } catch (\Exception $e) {
